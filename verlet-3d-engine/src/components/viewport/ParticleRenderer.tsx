...
import { Mesh, SphereGeometry, MeshStandardMaterial, Vector3 as ThreeVector3, Raycaster, Camera, Plane } from 'three';
...
export function ParticleRenderer({ particle }: ParticleRendererProps) {
  ...
  const isMultiSelecting = useEngineStore((state) => state.isMultiSelecting);
  ...
  }, [particle.color, isSelected, isMultiSelected, isConstraintStart, canFinishConstraint, isBeingDragged, isEditMode]);
  ...
  const handleClick = (e: any) => {
    ...
    if (isCreatingConstraint) {
      ...
      // Single click selects particle and shows property panel (only when not in edit mode)
      selectParticle(particle.id);
    }
  };
  ...
  const handleDoubleClick = (e: any) => {
    e.stopPropagation();
    if (!isCreatingConstraint && !isEditMode) {
      // Double-click starts constraint creation (only when not in edit mode)
