...
export class VerletEngine {
  ...
  update(deltaTime?: number): void {
    ...
    this.particles.forEach(particle => {
      if (!particle.fixed) {
        const gravityForce = Vector3.from(this.config.gravity).multiply(particle.mass);
        particle.applyForce(gravityForce);
        
        // Debug first few frames
        if (this.time < 0.1) {
          console.log(`Frame ${Math.floor(this.time/dt)} - Before update - Particle ${particle.id}:`, {
            position: { ...particle.position },
            previousPosition: { ...particle.previousPosition },
            acceleration: { ...particle.acceleration },
            velocity: {
              x: particle.position.x - particle.previousPosition.x,
              y: particle.position.y - particle.previousPosition.y,
              z: particle.position.z - particle.previousPosition.z
            }
          });
        }
      }
    ...
    this.particles.forEach(particle => {
      ...
      // Debug first few frames after update
      if (!particle.fixed && this.time < 0.1) {
        console.log(`Frame ${Math.floor(this.time/dt)} - After update - Particle ${particle.id}:`, {
          position: { ...particle.position },
          previousPosition: { ...particle.previousPosition },
          velocity: {
            x: particle.position.x - particle.previousPosition.x,
  ...
  private applyGroundCollision(particle: Particle): void {
    ...
    if (particleBottom < groundY) {
      ...
      if (velocityY < -0.005) { // Small threshold to prevent micro-bounces
        const restitution = 0.75; // Bounce factor (higher = more bouncy)
        ...
        const friction = 0.95; // More friction on bounce
  ...
  resume(): void {
    this.isPaused = false;
    this.time = 0; // Reset time for fresh debugging
    
    // Reset all particles to ensure they can move when resuming
    this.particles.forEach(particle => {
      if (!particle.fixed) {
        console.log(`Before resume - Particle ${particle.id}:`, {
          position: { ...particle.position },
          previousPosition: { ...particle.previousPosition },
          velocity: {
            x: particle.position.x - particle.previousPosition.x,
        ...
        console.log(`After resume - Particle ${particle.id}:`, {
          position: { ...particle.position },
          previousPosition: { ...particle.previousPosition },
          velocity: {
            x: particle.position.x - particle.previousPosition.x,
