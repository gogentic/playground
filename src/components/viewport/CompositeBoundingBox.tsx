...
export function CompositeBoundingBox({ composite }: CompositeBoundingBoxProps) {
  ...
  useEffect(() => {
    const updateBounds = () => {
      try {
        ...
        console.error('[CompositeBoundingBox] Error updating bounds:', error);
      }
    };
  ...
  return (
    <group position={[bounds.center[0], bounds.center[1], bounds.center[2]] as [number, number, number]}>
      ...
      >
        <boxGeometry args={[bounds.size[0], bounds.size[1], bounds.size[2]] as [number, number, number]} />
