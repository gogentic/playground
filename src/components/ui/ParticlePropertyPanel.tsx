...
export function ParticlePropertyPanel() {
  ...
  const [compositeRadius, setCompositeRadius] = useState(0.15);
  ...
  const composite = selectedCompositeId ? composites.get(selectedCompositeId) : null;

  // Update form when composite selection changes
  useEffect(() => {
    if (composite) {
      const particles = composite.getParticles();
      if (particles.length > 0) {
        // Get the average radius from all particles
        const avgRadius = particles.reduce((sum, p) => sum + p.radius, 0) / particles.length;
        setCompositeRadius(Math.round(avgRadius * 100) / 100);
      }
    }
  }, [composite]);
  ...
  // Live preview effect for composite radius
  useEffect(() => {
    if (composite && livePreview) {
      const particles = composite.getParticles();
      particles.forEach(p => {
        p.radius = compositeRadius;
      });
    }
  }, [composite, livePreview, compositeRadius]);

  // Live preview effect for individual particle - apply changes in real-time
  useEffect(() => {
  ...
  const applyCompositeRadius = () => {
    if (!composite) return;
    
    const particles = composite.getParticles();
    particles.forEach(particle => {
      particle.radius = compositeRadius;
    });
  };
  ...
  // Render composite properties panel
  if (composite) {
    return (
      <div 
        ref={panelRef}
      ...
      >
        ...
        >
          <h3>Composite Properties: {composite.name}</h3>
        ...
        <div className="property-section">
          ...
          <div className="property-row">
            ...
            <span>{composite.getParticles().length}</span>
          </div>

          <div className="property-row">
            <label>Constraints:</label>
            <span>{composite.getConstraints().length}</span>
          </div>
          
          <div className="property-row">
            <label>All Radii:</label>
            <input
              type="number"
              value={compositeRadius}
              onChange={(e) => setCompositeRadius(parseFloat(e.target.value))}
              min="0.1"
              max="2"
              step="0.1"
            />
            {!livePreview && (
              <button onClick={applyCompositeRadius} className="btn btn-secondary" style={{ marginLeft: '10px' }}>
                Apply
              </button>
            )}
          </div>
  ...
  // Render particle properties panel
  return (
    <div 
      ref={panelRef}
      className={`particle-property-panel ${isDragging ? 'dragging' : ''}`}
